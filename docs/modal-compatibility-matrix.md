...
**Provenance:**
- Updated the §E `modal.NetworkFileSystem` row (2026-08-15, calque#91
  Workstream B): moved from ⬜ (not modeled — leak only) to ✅ (a real,
  bring-your-own EFS mount over NFS). See the row itself for the
...
| `gpu=` **card substitution** (`calque real --allow-card-swap`) | — (calque-side capability, no Modal equivalent) | 🧊 (curated, one entry at a time) | ✅ (calque#178) an explicit opt-in flag substitutes a `CleanSwap` site's asked-for card for a VERIFIED cheaper alternative from `internal/target.CardSwapFor`'s curated table — currently one entry, `A100-80GB` → `RTX PRO 6000` (AWS has no single-GPU A100-80GB instance at all; `p4de.24xlarge` is an 8-GPU box). Verified on real g7e.2xlarge spot hardware running earth2studio's AIFS model end-to-end (real weight load, real live GFS data, real inference rollout — reported `SUCCESS`) for AI-Almanac's `forecasts_app.py`. Off by default; the asked-for card always passes through unchanged unless the flag is set AND a verified entry exists — no cost-search, no guessing. | Each table entry is narrow and per-workload-class; absence means "not yet verified," never "assume it's fine." | closed |
...
| `image=<var>` | Per-function image override. | 🔥 | ✅ resolved per-callable (calque#174), not app-wide. A direct `x = modal.Image....` chain resolves as-is; a **trivial** zero-arg factory function (no branching, one unconditional return — e.g. AI-Almanac's `blending_app.py`'s `_image()`) is inlined and resolved too (calque#175). A specific **parameterized-factory-in-a-for-loop** shape also resolves now (calque#179): `IMAGES = {k: factory(v) for k, v in D.items()}` followed by `for k, v in IMAGES.items(): @app.function(name=f"fn_{k}", image=v) def fn(...): ...` — the exact idiom AI-Almanac's `forecasts_app.py` uses for its per-environment earth2studio images — expands into one real `ir.Function` per (key, value) pair, with the factory's own local `sep.join(list)`-into-f-string substitution folded correctly per iteration. A factory that branches, or is called/parameterized in any OTHER shape (not this specific for-loop-over-dict-comprehension pattern), still can't be resolved and is loudly leaked instead of silently substituting the wrong image (calque#76/#179's own documented non-goals). | Branching factories, and parameterized factories outside the specific for-loop/dict-comprehension shape calque#179 models, remain an honest gap, not a silent one. | closed |
...
| `Image.from_registry(...)` | Pull an existing image; `linux/amd64` required. | 🟡 | ✅ resolves at parse time (dry-run Dockerfile FROM line); a `--script` real run against the picked unit actually `docker pull`s this exact ref when it's a bare reference with nothing layered on top (calque#176) — ECR hostnames authenticate via `aws ecr get-login-password`, other registries (e.g. AI-Almanac's `app.py`'s real GCP Artifact Registry `ROMP_IMAGE_URI` default) pull anonymously. When the chain has steps LAYERED on top of this base (`.pip_install(...)`/`.add_local_file(...)` etc. — e.g. `app.py`'s `benchmark_image`, or a from-scratch `debian_slim()...` chain with no pullable base at all, e.g. `blending_app.py`'s `_image()`), calque instead renders the Dockerfile (`internal/image.Render`, the same renderer `--dry-run` already uses) and runs `docker build` ON THE ACQUIRED INSTANCE itself before `docker run` (calque#177) — no ECR round-trip, no ambient Docker requirement on the caller's machine, no second/throwaway instance. | `add_local_*`/`from_dockerfile` steps within the built chain that calque can't stage into the build context are still leaked separately (unchanged, see `internal/image`'s own per-step leaks). | [#176](https://github.com/spore-host/calque/issues/176)/[#177](https://github.com/spore-host/calque/issues/177) closed |
...
| `Image.from_aws_ecr(...)` | Pull from ECR; `secret=` carries IAM/OIDC. | ⚪ | ✅ resolves at parse time + emits an `integration_edge` leak noting IAM pull-permission needs; a `--script` real run pulls this ref for real too, same authenticated pull-or-build path as `from_registry` above (calque#176/#177). | Same as `from_registry` above. | [#176](https://github.com/spore-host/calque/issues/176)/[#177](https://github.com/spore-host/calque/issues/177) closed |
...
| `modal.NetworkFileSystem` (deprecated, being removed) | **Live-shared** filesystem — no commit/reload cycle, closer to EFS/NFS than Volume's snapshot model. | 🧊 (deprecated, Modal steers users to Volume) | ✅ (calque#91 Workstream B) a real `NetworkFileSystem.from_name(name)` used as a `network_file_systems={mount: nfs}` value (a SEPARATE decorator kwarg from `volumes=`, never nested inside it) resolves to a real EFS-over-NFS mount, bring-your-own: `tools/pyast/pyast.py`'s `_network_file_systems_map`/`self.network_file_systems` track the binding structurally (mirroring `Volume.from_name`'s own zero-leak-on-binding posture), `internal/parse/parse.go` decodes them into `ir.Function`/`ir.Class.NetworkFileSystems`, and `internal/plan/efs.go`'s `DiscoverEFSFilesystem` finds the pre-provisioned EFS filesystem by matching a `calque:nfs-name=<name>` tag (calque NEVER auto-creates an EFS filesystem — `create_if_missing=True` is a distinct leak, not a blocker), `ResolveMountTargetsForAZs` narrows the acquirer's AZ sweep to AZs with live mount-target coverage (a hard error, not a leak, if narrowing empties it), `EnsureNFSSecurityGroup` creates/finds a self-referential NFS/2049-ingress security group attached to the launched instance via `spawnaws.LaunchConfig.SecurityGroupIDs`, and `NFSMountCommands` renders the on-instance `mount -t nfs4` invocation (spliced into the bootstrap script before `@enter` runs, via `internal/exec.BootstrapConfig.NFSMountLines`). IAM (`elasticfilesystem:ClientMount`/`ClientWrite`) is explicitly OUT of scope for this pass — the EFS filesystem's own access policy/security-group pairing is the operator's responsibility. | No auto-create (bring-your-own only, matching Modal's own `create_if_missing=False` default). No IAM grant — a filesystem-level access policy that requires `ClientMount`/`ClientWrite` must already permit the instance's role/network path. | [#91](https://github.com/spore-host/calque/issues/91) (Workstream B closed) |
| `modal.CloudBucketMount` | Direct S3/R2/GCS mount via `mountpoint-s3` — no append writes, no seek+write, must open in truncate mode, no rename. | 🧊 | ✅ (calque#91 Workstream A) a real `CloudBucketMount(bucket_name, key_prefix=, read_only=)` used INLINE as a `volumes=` value (the real Modal idiom — constructed directly in the dict, not assigned to a variable first) resolves to a real mountpoint-s3 mount against the SCRIPT'S OWN S3 bucket: `tools/pyast/pyast.py`'s `_cloud_bucket_mount` extracts `bucket_name`/`key_prefix`/`read_only`, `internal/parse/parse.go` decodes them into `ir.Function`/`ir.Class.CloudBucketMounts`, and `internal/plan/cloudbucketmount.go`'s `MountCommands` renders the on-instance `mount-s3` invocation (spliced into the bootstrap script before `@enter` runs, via `internal/exec.BootstrapConfig.CloudBucketMountLines`); `internal/plan.RealRunPolicy`'s `extraBuckets` param grants the instance role read/write/list on that bucket, separate from calque's own `--bucket` staging area. `secret=` is recognized but NOT honored (the instance's own IAM role is used instead) — leaked distinctly. `bucket_endpoint_url=`/`requester_pays=`/`force_path_style=` are NOT supported — leaked distinctly; mounting is against AWS S3 with default settings only. A `bucket_name` that isn't a string literal still falls back to the pre-existing "recognized but not modeled" leak. | R2/GCS-backed CloudBucketMounts (`bucket_endpoint_url=`) are NOT reproduced — AWS S3 only. No live-Modal-managed credential rotation via `secret=`; the instance's own IAM role is the only credential path. | [#91](https://github.com/spore-host/calque/issues/91) (Workstream A closed; see the row below for Workstream B, `modal.NetworkFileSystem`) |
...
12. Lower-priority/rare, still open: `modal.Dict`/`Queue`,
    `@modal.batched`, `App.include`/`.deploy`/`.run` lifecycle nuances.
    (`cloud=` closed separately, calque#91's own §C fix; `modal.Cron`/
    `Period` object-form *recognition* also closed under calque#91 — see
    §H — though actually executing on a schedule remains out of scope.
    `modal.CloudBucketMount` is now REAL — see §E — closed as calque#91
    Workstream A; `modal.NetworkFileSystem` is now ALSO REAL — see §E —
    closed as calque#91 Workstream B (bring-your-own EFS-over-NFS, no
    auto-create). Both concrete real-AWS-mapping workstreams calque#91 was
    tracking are now done; `modal.Dict`/`Queue`/`App.include` remain
