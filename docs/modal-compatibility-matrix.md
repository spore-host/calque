...
| `.local(*args, **kwargs)` | Runs in the CALLER's own process/container — no new container, only locally-available resources apply. | 🧊 (rare — mostly same-class intra-container calls or entrypoint-local testing, NOT general pipeline chaining) | 🟨 (fixed 2026-08-07, calque#81) recognized and leaked at the call site: calque ships only ONE function/method's body verbatim (the picked warm unit), so a sibling referenced via `.local()` is not in scope and will `NameError` at runtime — leaked explicitly rather than left as a mystery crash. Verified against the real `blending_app.py` (6 call sites, all detected). | Recognition-only; full support would require shipping the callee's body alongside the warm unit's, which is a bigger change (multi-function body bundling) not attempted here. | [#81](https://github.com/spore-host/calque/issues/81) (recognition, closed) + [#92](https://github.com/spore-host/calque/issues/92) (full inlining) |
| `.spawn(*args, **kwargs)` → `FunctionCall` handle | Non-blocking; `FunctionCall.object_id` is a persistable string, reconstructable via `.from_id()` from a different process; results retrievable for 7 days post-completion. | ⚪ (~7% of files; common at web/bot/CLI boundaries — "trigger and poll") | 🟨 (fixed 2026-08-07, calque#88) each `.spawn()`'d callable is now CLASSIFIED (`ir.InvokeSpawn`) and findable via new `ir.App.FindFunction`/`FindClass` lookup helpers, still leaked as not-yet-executed. Confirmed no regression: a `.map()`'d callable still wins warm-unit selection over a merely-spawned one (rank precedence). Verified against the real `forecasts_app.py` — one spawn site there targets a dynamic subscript expression (`SEASON_BUNDLE_FNS[...]spawn(...)`), which the AST can't statically name; the leak still fires, just without a specific callable name, an honest limitation not a bug. | calque still doesn't reproduce Modal's real decoupled contract (persistable handle, 7-day retention, cross-process `.get()`) — confirmed as explicitly out of scope, matching §18. Executing even the narrower "block-and-wait fan-out" needs its own driver — a real structural change to `run()`'s single-warm-unit model, with no offline test path (matches `fleetrun.go`'s existing untested-without-AWS-spend precedent). | [#88](https://github.com/spore-host/calque/issues/88) closed (classification); driver tracked as [#97](https://github.com/spore-host/calque/issues/97) |
