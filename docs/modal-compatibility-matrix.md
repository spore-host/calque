...
| `App(image=..., secrets=..., volumes=...)` (app-level defaults) | Inherited by every Function/Cls unless overridden. | ⚪ | `volumes=`/`secrets=`: ✅ (fixed calque#168) — `ir.App.DefaultVolumes`/`DefaultSecrets` inherited by any Function/Class declaring neither, same App→class→method chain the existing gpu=/volumes= class→method fallback already used one level down. `image=`: 🟨 still recorded-as-a-leak only — real per-function image RESOLUTION is a separate, larger gap (`resolveImage` picks ONE image var for the whole script; see the multiple-image-definitions row below), not just an app-level fallback. | Before #168, `volumes=`/`secrets=` were silently dropped with **zero leak at all** (confirmed via live repro) — worse than `image=`, which at least surfaced a generic leak. `image=`'s own leak is now distinctly worded so it isn't confused with the now-fixed `volumes=`/`secrets=` cases. | [#168](https://github.com/spore-host/calque/issues/168) closed (volumes=/secrets=); image= tracked separately as [#174](https://github.com/spore-host/calque/issues/174) |
