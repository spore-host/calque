...
| `Image.from_registry(...)` | Pull an existing image; `linux/amd64` required. | 🟡 | ✅ resolves at parse time (dry-run Dockerfile FROM line); a `--script` real run against the picked unit now actually `docker pull`s this exact ref instead of forcing host-mode (calque#176) — ECR hostnames authenticate via `aws ecr get-login-password`, other registries (e.g. AI-Almanac's `app.py`'s real GCP Artifact Registry `ROMP_IMAGE_URI` default) pull anonymously. | Any dependency LAYERED on top of this base in the script's own `.image` chain (`.pip_install(...)`/`.add_local_file(...)` etc. called on the `from_registry` result — e.g. `app.py`'s `earth2studio`/`gcsfs`/`zarr`) is NOT built; still needs `--pip`/`--stage-file` until calque#177's build-decision lands. | [#176](https://github.com/spore-host/calque/issues/176) closed (base-image pull only) |
...
| `Image.from_aws_ecr(...)` | Pull from ECR; `secret=` carries IAM/OIDC. | ⚪ | ✅ resolves at parse time + emits an `integration_edge` leak noting IAM pull-permission needs; a `--script` real run pulls this ref for real too, same authenticated path as `from_registry` above (calque#176). | Same layered-deps caveat as `from_registry` above. | [#176](https://github.com/spore-host/calque/issues/176) closed (base-image pull only) |
