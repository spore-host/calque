...
| App-level `secrets=`/`volumes=` defaults (set once on `modal.App(...)`, inherited by every function) | Inherited (calque#168) by any Function/Class that doesn't set its own — same App→class→method fallback chain the class-level gpu=/volumes= inheritance already used. | ✅ | Nothing for the common case. Before calque#168, both were silently dropped with **zero leak at all** — if you're on a build predating that fix, treat this row as the ❌ case below applied to secrets=/volumes= too. |
| App-level `image=` default (set once on `modal.App(...)`, inherited by every function) | Recorded as a leak, never applied to any function that doesn't set its own — a distinct, larger gap from secrets=/volumes= above: real per-function image *resolution* is unimplemented (calque picks ONE image variable for the whole script regardless of who references it), not just a missing app-level fallback. Tracked as [#174](https://github.com/spore-host/calque/issues/174). | ❌ | **Make every function's `image=` explicit.** A function relying on the app-level default silently gets no image at all today — this remains the sharpest gap in this table for `image=` specifically. |
