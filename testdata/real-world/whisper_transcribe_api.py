...
# Origin: mharrvic/fast-audio-video-transcribe-with-whisper-and-modal, file api/main.py
# https://github.com/mharrvic/fast-audio-video-transcribe-with-whisper-and-modal/blob/main/api/main.py
#
# Verbatim real-world source (only this header block added; relative package
# imports `from . import audio, config, video` are left as-is and will not
# resolve standalone -- consistent with how calque treats unavailable local
# imports elsewhere in the corpus). Legacy Modal script: `Stub`, deprecated
# `modal.NetworkFileSystem.persisted(...)`, `Dict.new()`, old-era
# `keep_warm=1` autoscaling kwarg, a served `@asgi_app` FastAPI factory
# function, `.starmap()` with an explicit `kwargs=` dict (not just a plain
# tuple iterable), `stub.function(...).call(...)` (the pre-1.0 spelling of
# `.remote(...)`), and `from modal import container_app` (removed API).
