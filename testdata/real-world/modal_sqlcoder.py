...
# Origin: dcalaprice/modal-sqlcoder, file sql_generation_inference.py
# https://github.com/dcalaprice/modal-sqlcoder/blob/main/sql_generation_inference.py
#
# Verbatim real-world source (only this header block added). A pre-1.0 Modal
# script (legacy `Stub`, `gpu.A100(...)` object API, `__enter__`/`__exit__`
# dunder lifecycle instead of @modal.enter/@modal.exit, allow_concurrent_inputs/
# container_idle_timeout kwargs, an async generator @method, and an unused
# `Mount` import) hosting a TGI subprocess server inside a Modal class.
