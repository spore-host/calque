...
# Origin: tokenbender/avataRL, file modal_train.py
# https://github.com/tokenbender/avataRL/blob/main/modal_train.py
#
# Verbatim real-world source (only this header block added). A real
# multi-node distributed training script: `@modal.experimental.clustered(
# n_nodes, rdma=USE_RDMA)` STACKED with `@app.function(gpu=f"{GPU_TYPE}:
# {n_proc_per_node}", ...)` -- confirmed via grep that calque has ZERO
# recognition of `@modal.experimental.clustered` anywhere in its parser or
# GPU guard (calque#150's top predicted finding: a multi-node clustered
# training decorator's node-count multiplier is invisible to the §7 GPU
# coupling guard, which only inspects the per-node `gpu="H100:8"` card
# count in the SAME decorator's own gpu= value). Also exercises a
# multi-GPU f-string gpu= spec (`f"H100:{n_proc_per_node}"`) and an
# upgrade-pin suffix on a MULTI-gpu spec (`f"H100!:{n_proc_per_node}"` --
# a compound case: both flag_multi AND the `!` upgrade-pin marker in one
# spec). NOT a real-AWS validation target -- real execution needs actual
# multi-GPU H100/H200 nodes; analyze/dry-run only, per calque#150 Part 5.
