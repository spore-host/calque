...
# Origin: RomeroLab/alphafast, files modal/af3_predict.py + modal/config.py
# https://github.com/RomeroLab/alphafast/blob/main/modal/af3_predict.py
# https://github.com/RomeroLab/alphafast/blob/main/modal/config.py
#
# Copyright 2026 Romero Lab, Duke University. Licensed under CC-BY-NC-SA 4.0
# (https://creativecommons.org/licenses/by-nc-sa/4.0/) -- this file is part
# of AlphaFast, a derivative work of AlphaFold 3 by DeepMind Technologies
# Limited. Preserved from the original per its own license terms.
#
# Verbatim real-world source, merged into one file (config.py's constants
# inlined in place of `from config import (...)`; only this header block and
# the merge are changes). A real @cls+@modal.enter()+plain-@modal.method()
# GPU inference pipeline at production scale: `modal.Dict.from_name(...)`/
# `modal.Queue.from_name(...)` used as REAL hot-path data-transfer primitives
# inside a method body (not just declared and ignored -- calque#150's plan
# flagged this as untested: does calque cleanly leak a bare reference to a
# Dict/Queue-valued module constant, or ship it verbatim into a runtime
# crash?), a bare-Name (not even an f-string) non-literal `gpu=_CONSUMER_GPU`
# resolved via a helper function from sys.argv at import time, and
# `Image.from_dockerfile(path=, context_dir=)` (already closed, calque#84).
# `InferenceWorker.warmup` (`@modal.method()`, body is `return "warm"`) is
# the cheapest possible real-AWS validation target in this file -- zero
# third-party deps, no GPU-dependent code path -- see calque#150's Part E.
